...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Create New Module</h2>
            <p class="text-slate-500 text-sm">Add a new learning module to the platform.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm">
        <form method="POST" action="{{ route('admin.modules.store') }}" class="p-6 md:p-8 space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" required
                    ...
                    placeholder="Provide a detailed description of the module..."></textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="duration_minutes" id="duration_minutes" required min="1" max="10080"
                        class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                        placeholder="e.g., 120">
                ...
                <div>
                    ...
                    <input type="number" name="lesson_count" id="lesson_count" required min="0"
                        class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                        placeholder="e.g., 5">
                    @error('lesson_count')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_active" id="is_active" value="1" checked
                    class="w-5 h-5 text-blue-600 border-slate-300 rounded focus:ring-blue-500">
                <label for="is_active" class="text-sm font-medium text-slate-700">Make module active immediately</label>
            </div>
